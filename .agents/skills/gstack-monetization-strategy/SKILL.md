...
description: |
  가이드라인: 비즈니스 모델(BM)과 결제 아키텍처 수립 전반을 리드하며, 프리미엄(Freemium) 정책 및 과금 모델 설계를 전담합니다.
---
...
echo '{"skill":"monetization-strategy","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.gstack/analytics/skill-usage.jsonl 2>/dev/null || true
...
# 비즈니스 수익화(Monetization) 기획 가이드라인

## 1. Freemium 및 과금 모델(BM) 설계 원칙
- **가치 전환선(Paywall) 설정:** 제품의 기본 가치는 무료로 경험하게 하되, "반복적이고 핵심적인 병목을 해결해주는 순간"에 유료 과금벽(Paywall)을 배치하도록 설계합니다.
- **티어링(Pricing Tier):** 가격 정책 설계 시, 3단 티어(Basic - Pro - Enterprise) 구조를 기본안으로 제안하고, 가운데(Center)의 상품이 가장 매력적으로 보이도록 디코이(Decoy) 이펙트를 적용합니다.

## 2. 결제 시스템 연동 가이드
- **글로벌/로컬 타겟팅 구분:** 타겟이 글로벌일 경우 **Stripe** 연동 계획 체계를 짜고, 로컬(국내) 중심일 경우 **PortOne(포트원) / 토스페이먼츠**의 API 연동을 우선 기획합니다.
- **데이터 흐름 구조:** 결제 발생 시 [DB 저장] -> [외부 API 결제 상태 폴링] -> [유저 등급 변경 웹훅] 과정의 아키텍처 다이어그램을 시스템 설계 시 반드시 첨부합니다.

## 3. 이탈률(Churn) 방어
- 구독 해지나 결제 실패 시 리텐션을 높이기 위한 [할인 오퍼 제공] 또는 [피드백 폼 연동] 구조를 함께 코드로 구현할 것을 제안합니다.
