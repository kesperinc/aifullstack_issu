...
description: |
  프론트엔드 코드를 기획하고 작성할 때 다음의 최신 트렌드 디자인 원칙 (Glassmorphism 등)을 전역으로 적용합니다. UI/UX 구현 시 사용하세요.
---
...
echo '{"skill":"design-system-rules","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.gstack/analytics/skill-usage.jsonl 2>/dev/null || true
...
2. **필수 CSS/스타일 속성:**
   - **Background:** 반투명한 배경색 (예: `rgba(255, 255, 255, 0.1)` 또는 `rgba(0, 0, 0, 0.2)`)
   - **Backdrop-filter:** 배경 블러 효과 필수 (예: `backdrop-filter: blur(10px);`)
   - **Border:** 빛 반사 효과를 위한 1px의 미세하고 밝은 테두리 (예: `border: 1px solid rgba(255, 255, 255, 0.2);`)
   - **Shadow:** 공간감을 띄우기 위한 부드러운 드롭 섀도우 (예: `box-shadow: 0 8px 32px 0 rgba(31, 38, 135, 0.37);`)
3. **색상 팔레트:** 차분한 파스텔 톤이나 그라데이션 배경을 기본 바탕으로 설정하여 글래스모피즘이 돋보이게 구성할 것.
4. **디자인 토큰화:** 색상, 블러 수치 등은 하드코딩하지 말고 CSS 변수(Variables)나 테마 설정 파일(`theme.py`, `tailwind.config.js` 등)로 분리할 것.
5. Python 웹 UI의 디자인은 Streamlit, Gradio 등을 사용할 수 있음. "Streamlit을 활용하는 경우 st.markdown(unsafe_allow_html=True)을 활용해서 전체 배경과 컨테이너에 글래스모피즘 Custom CSS를 주입하는 코드를 작성할 것. Gradio 등도 이에 상응하는 코드를 작성하여 반영할 것.
6. 웹 프론트엔드는 React, Vue, HTML/CSS을 기본으로 구현이 쉬운 방법을 택하여 깔끔하게 구현하고, 어떠한 툴을 사용하더라도 앞에 필수 스타일 속성을 반영하여 구현할 것.
7. Python 데스크톱 UI는 PyQt, PySide 등을 프로젝트의 성격에 맞추어 구현이 쉽고 깔끔한 구현이 가능한 스타일을 해야 하며, 깔끔한 스타일의 구현이 어려우면 PyQt의 FramelessWindowHint와 반투명 속성(setAttribute)을 사용해서 모던한 글래스모피즘 창을 설계해서 디자인을 방향을 잡을 것. 
