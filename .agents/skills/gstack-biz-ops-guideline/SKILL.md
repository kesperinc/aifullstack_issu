---
name: biz-ops-guideline
description: |
  가이드라인: 제품 출시 후 운영 과정에서의 커뮤니케이션, 마케팅, CS 관리를 전담하는 에이전트 룰과 템플릿 제안을 수행합니다.
---
...
echo '{"skill":"biz-ops-guideline","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.gstack/analytics/skill-usage.jsonl 2>/dev/null || true
...
# 비즈니스 운영 관리(Biz-Ops) 가이드라인

## 1. Marketing (마케팅 및 그로스)
- **타겟 메시징:** 모든 마케팅 카피는 전문 용어를 배제하고, "고객이 얻을 수 있는 최종 가치(Value Proposition)"를 1문장으로 명확히 표현해야 합니다.
- **콘텐츠 발행 원칙:** 블로그, SEO 아티클 작성 시 일관성 있는 톤앤매너(친근하지만 전문적인 어조)를 유지하고, Call-To-Action (CTA) 버튼 위치와 문구를 항상 기획안 포맷 하단에 포함시킵니다.
- **초기 론칭 루틴:** 제품 론칭 시 Product Hunt, Hacker News, X(구 트위터) 등 주요 채널별 맞춤형 포스팅 템플릿(글자 수 제한, 적절한 태그 포함)을 먼저 제안합니다.

## 2. Sales (세일즈 프레임워크)
- **퍼널(Funnel) 점검:** 사용자가 방문하여 결제까지 가는 모든 단계에서의 클릭 전환율(CTR)을 높이기 위한 A/B 테스트 전략을 우선적으로 제시합니다.
- **콜드 메일(Cold Email):** 세일즈 콜드 메일을 작성할 땐 [어텐션(문제 공감)] -> [솔루션 제시] -> [행동 유도]의 3단계를 시각적으로 짧게 구성합니다.

## 3. CS (고객 성공 및 지원)
- **FAQ 및 클레임 방어:** 고객 클레임 응대 메시지 작성 시, [1. 공감 및 사과] -> [2. 문제 원인 설명] -> [3. 명확한 보상 및 해결 가이드] 구조를 지켜서 작성합니다. 변명을 덧붙이지 않습니다.
- **매뉴얼화:** 반목되는 질문 발견 시, 유저 친화적인 도움말 문서를 선행적으로 작성해 제안합니다.

## 4. Operations (운영 및 지표 추적)
- **지표 대시보드:** 항상 KPI (순익, 전환율, 이탈률 CHURN 등) 개선을 목표로 하는 시각에서 기술적 피드백을 제공합니다. 
- **인프라 비용:** 서비스 운영에 들어가는 서버/토큰 API 비용 최적화 이슈를 수시로 모니터링할 도구를 고민합니다.
