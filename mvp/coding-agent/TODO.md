...
- [x] 로컬 데스크톱 Runner 스크립트 작성 (`mvp/coding-agent/src/desktop_runner.py`)
- [x] Vibe Coding 요구사항 파서 & 다중 파일 자율 생성기 작성 (`mvp/coding-agent/src/vibe/`)
- [x] OpenRouter API Key 시크릿 감지 Pre-commit Hook 및 스크립트 적용 (`mvp/coding-agent/scripts/check_secrets.py`)

### Stage 2: GCP 기반 멀티 테넌트 & MCP 라우터 확장
- [ ] GCP 인프라 구축 및 개발자 테넌트별 Docker 샌드박스 할당기
- [x] MCP (Model Context Protocol) JSON-RPC 게이트웨이 포트(3000) 바인딩
- [x] VS Code (Continue.dev) 및 Agentic CLI (Antigravity/Claude Code) MCP 커넥터 연동
...
- [x] OpenShift AI (SNO) vLLM 엔드포인트 호환 어댑터 1-Click 스위칭 포팅
- [x] 샌드박스 자율 `pytest` 실행 및 에러 셀프코렉션(Self-Correction) 라이브 부스 연출
